...
const card = {
  ...
  getIdCard: function (card_number, callback) {
  ...
  add: function (add_data, callback) {
    bcrypt.hash(add_data.pin, saltRounds, function (err, hashedPin) {
      return db.query(
        'insert into card (card_number,pin,id_user) values(?,?,?)',
        [add_data.card_number, hashedPin, add_data.id_user],
        callback);
    });
  },
  ...
  update: function (id, update_data, callback) {
    bcrypt.hash(update_data.pin, saltRounds, function (err, hashedPin) {
      return db.query(
        'update card set card_number=?,pin=?,id_user=? where id_card=?',
        [update_data.card_number, hashedPin,
